function combineADSB_Three(ADSB_Band1, ADSB_Band2, ADSB_Band3)
    % Extract parts from each frame
    ...
    df_ca3 = ADSB_Band3(1:2);     % Downlink Format and Capability
    icao3 = ADSB_Band3(3:8);      % ICAO address
    message3 = ADSB_Band3(9:22);  % Message part
    parity3 = ADSB_Band3(23:28);  % Parity part
    ...
    ADSB_Band3_info = struct('full_frame', ADSB_Band3, 'df_ca', df_ca3, 'icao', icao3, 'message', message3, 'parity', parity3);
    
    % Verify that df_ca and ICAO are the same for all three frames
    if ~(strcmp(df_ca1, df_ca2) && strcmp(df_ca2, df_ca3)) || ~(strcmp(icao1, icao2) && strcmp(icao2, icao3))
    ...
    for i = 1:length(message1)
        ...
        sum_val = val1 + val2 + val3 + carry;
        carry = floor(sum_val / 16);
        digit = mod(sum_val, 16);
        
        % Append digit to message
        message = [message, dec2hex(digit, 1)];
    end
    ...
    disp(['ADSB_Band3: ', ADSB_Band3]);
    % disp(['  Full Frame: ', ADSB_Band3]);
    % disp(['  DF/CA: ', df_ca3]);
    % disp(['  ICAO: ', icao3]);
    % disp(['  Message: ', message3]);
    % disp(['  Parity: ', parity3]);
    % disp(' ');
